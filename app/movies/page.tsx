...
export default function MoviesPage() {
  ...
  const { movies, trendingMovies, topRatedMovies, newReleases } = useAppSelector((state) => state.movies)
  ...
  const filteredMovies = useMemo(() => {
    ...
    if (moodFilter) {
      ...
      if (moodData) {
        filtered = filtered.filter(movie =>
          moodData.genres.some(genre => movie.genres.includes(genre))
        )
      }
    }
    ...
    if (year) {
      filtered = filtered.filter(
        (movie) => new Date(movie.releaseDate).getFullYear() === year
      )
    }
    ...
    return filtered
  }, [movies, trendingMovies, topRatedMovies, newReleases, activeTab, user?.favorites, searchQuery, moodFilter, genres, year, rating, sortBy])

  // Quick actions for movies
  const getQuickActions = (movie: Movie) => [
    {
      id: 'watchlist',
      label: user?.watchlist?.includes(movie.id) ? 'Remove from Watchlist' : 'Add to Watchlist',
      icon: FiBookmark,
      action: () => {
        dispatch(addNotification({
          id: Date.now().toString(),
          type: 'success',
          title: user?.watchlist?.includes(movie.id) ? 'Removed from Watchlist' : 'Added to Watchlist',
          message: `${movie.title} ${user?.watchlist?.includes(movie.id) ? 'removed from' : 'added to'} your watchlist`,
    ...
    {
      id: 'favorite',
      label: user?.favorites?.includes(movie.id) ? 'Remove from Favorites' : 'Add to Favorites',
      icon: FiHeart,
      action: () => {
        dispatch(addNotification({
          id: Date.now().toString(),
          type: 'success',
          title: user?.favorites?.includes(movie.id) ? 'Removed from Favorites' : 'Added to Favorites',
          message: `${movie.title} ${user?.favorites?.includes(movie.id) ? 'removed from' : 'added to'} favorites`,
          timestamp: new Date().toISOString(),
          isRead: false
        }))
      }
  ...
  const handleRefresh = async () => {
    ...
    setTimeout(() => {
      ...
      dispatch(addNotification({
        id: Date.now().toString(),
        type: 'success',
        title: 'Movies Refreshed',
        message: 'Latest movies loaded!',
        timestamp: new Date().toISOString(),
        isRead: false
      }))
    }, 1500)
          ...
          >
            <div className="flex items-center gap-4">
              <p className="text-gray-400">
                Showing <span className="text-white font-medium">{filteredMovies.length}</span> {filteredMovies.length === 1 ? 'movie' : 'movies'}
                {searchQuery && (
                  ...
                  {viewMode !== 'grid' && (
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-gray-400 text-xs">•</span>
                        <span className={`text-xs px-2 py-0.5 rounded text-white bg-blue-600`}>
                          Movie
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {movie.genres?.slice(0, viewMode === 'compact' ? 2 : 3).map(genre => (
